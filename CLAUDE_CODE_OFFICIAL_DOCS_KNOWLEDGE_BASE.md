# Claude Code Official Documentation Knowledge Base

> Complete compilation of all official Claude Code documentation from docs.anthropic.com
...
13. [Terminal Configuration](#terminal-configuration)
...
- **Unix philosophy**: Claude Code is composable and scriptable. `tail -f app.log | claude -p "Slack me if you see any anomalies appear in this log stream"` works. Your CI can run `claude -p "If there are new text strings, translate them into French"` in a single line.

### Getting Started
To use Claude Code, you'll need an Anthropic account. Create one at anthropic.com.

#### For Solo Developers
- Set up Claude Code with a clear, comprehensive quickstart guide
- Once you're up and running, see common workflows to develop your skills further
- If you get stuck, we have a detailed troubleshooting guide
- The Anthropic team and community are also happy to help in our Discord server

#### For Teams
- Check out advanced setup for Claude Code on Amazon Bedrock and Claude Code on Google Vertex AI
- To deploy Claude Code for your company, see authentication, authorization, and permissions and Corporate proxy settings
- Engineers can integrate Claude Code into existing tools, or build new AI-powered coding experiences on top of the Claude Code SDK

### Claude Code is Enterprise Ready
Teams across industries trust Claude Code for mission-critical development work.

- **Hosted on AWS or GCP**: Deploy Claude Code using Amazon Bedrock or Google Vertex AI and keep data within your infrastructure
- **Enterprise-grade security and compliance**: Review Claude's security practices and supported compliance frameworks
...
1. **Explore Common Workflows** - Learn patterns for debugging, refactoring, and more
2. **Read about Interactive Mode** - Master keyboard shortcuts and advanced features
3. **Try IDE Integration** - Use Claude Code from VS Code or JetBrains
4. **Learn about Memory** - Teach Claude your preferences and project conventions

### Getting Help
- Run `/help` in Claude Code for built-in documentation
- Visit our troubleshooting guide for common issues
- Join the Anthropic Discord for community support
...
### Security Warning
"USE AT YOUR OWN RISK" - hooks can modify or delete files and require careful implementation. This comprehensive guide covers the essential aspects of Claude Code hooks, enabling you to customize and extend Claude Code's behavior to match your specific workflow requirements.
...
| `--resume`, `-r` | Resume a specific session by ID | `claude -r "session-id"` |
| `--continue`, `-c` | Continue most recent conversation | `claude -c` |
| `--input-format` | Set input format (text, stream-json) | `claude --input-format stream-json` |
| `--system-prompt` | Override default system prompt (print mode only) | `claude -p --system-prompt "You are..." "query"` |
| `--permission-mode` | Set permission mode | `claude --permission-mode strict` |
| `--permission-prompt-tool` | Specify custom permission prompt tool | `claude --permission-prompt-tool myTool` |
| `--dangerously-skip-permissions` | Skip all permission prompts | `claude --dangerously-skip-permissions` |
| `--mcp-config` | Load MCP servers from config | `claude --mcp-config ./mcp.json` |
| `--log-level` | Set logging level | `claude --log-level debug` |

### Key Flags Categorized

#### 1. Directory and Access
- `--add-dir`: Add working directories
- `--allowedTools`: Specify permitted tools without prompting
- `--disallowedTools`: Specify disallowed tools without prompting

#### 2. Input/Output Configuration
- `--print`, `-p`: Print response without interactive mode
- `--output-format`: Set output format (text, json, stream-json)
- `--input-format`: Set input format (text, stream-json)
- `--verbose`: Enable detailed logging
- `--log-level`: Set specific logging level
...
- `--system-prompt`: Override default system prompt (print mode only)

#### 4. Permission and Security
- `--permission-mode`: Set permission mode
- `--permission-prompt-tool`: Specify permission prompt tool
- `--dangerously-skip-permissions`: Skip permission prompts

#### 5. MCP Integration
- `--mcp-config`: Load Model Context Protocol servers from configuration
...
- Supports reverse search with `Ctrl+R`
- History expansion (`!`) is disabled by default

### Additional Features
- Configurable via `/terminal-setup`
- Supports IDE and terminal integrations
- Provides flexible input and editing capabilities
- Enabled via `/vim` command or permanent configuration
...
- Can execute bash commands with `!`
...
- Available when MCP servers are connected
...
- WebFetch
- Write
- And several others with varying permission requirements
...
Create custom notification hooks for advanced handling
...
Memory files are automatically loaded into Claude Code's context when launched, providing persistent project and personal configuration across sessions.
...
### Authentication
The primary authentication method is through the `ANTHROPIC_API_KEY` environment variable. Enterprise users can also use Amazon Bedrock or Google Vertex AI with appropriate environment variables.

### Model Selection
The documentation doesn't specify exact model names, but settings support a `model` configuration option to override defaults.

### Safety and Permissions
Claude Code includes a permission system for potentially dangerous operations. Tools like Bash and Edit require explicit permission, while Read operations are generally allowed.

### Session Management
Sessions are tracked and can be resumed using `--continue` or `--resume` flags. Session data includes conversation history and context.

### Context Management
Claude Code automatically manages context, but users can:
- Monitor usage with `/status`
- Compress context with `/compact`
- Clear visual display with `/clear`

### File Operations
Claude Code can read, write, and edit files with appropriate permissions. It includes safety features and logging for all file operations.

---

*This knowledge base represents a complete compilation of official Claude Code documentation as of 2025-07-19. For the most current information, always refer to the official documentation at docs.anthropic.com.*